defmodule DiscussWeb.AssistantChatChannelTest do
  ...
  test "assistant_chat:new_message broadcasts new message", %{socket: socket, chat: chat} do
    ...
    assert_broadcast "assistant_chat:new_message", %Message{content: "new message", chat_id: ^chat_id}
  end

  test "assistant_chat:message_viewed broadcasts updated message", %{socket: socket, chat: chat, user: user} do
  ...
  test "assistant_chat:load_more_messages returns next page", %{socket: socket, chat: chat, user: user} do
    first_part = insert_list(10, :message, %{chat: chat, sender: user})
      |> Enum.map(fn message -> message.id end)
      |> (fn ids -> Discuss.Repo.all(from m in Message, where: m.id in ^ids) end).()
    second_part = insert_list(20, :message, %{chat: chat, sender: user})
      |> Enum.map(fn message -> message.id end)
      |> (fn ids -> Discuss.Repo.all(from m in Message, where: m.id in ^ids) end).()
